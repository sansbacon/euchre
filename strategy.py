...
def make_bid(strategy, cards, bid_round, table_position, flipped_card):
    """Returns player's bid as tuple (bid, alone, suit)
    """
    ...
    if strategy == 'scorecard_simple':
        ...
        if best_score >= thresholds[bid_round][table_position][1]:
            return True, True, best_suit
        elif best_score >= thresholds[bid_round][table_position][0]:
            return True, False, best_suit
        else:
        ...
        return bid, alone, suit
    
    elif strategy == 'random':
        bid_prob = 0.2
        alone_given_bid_prob = 0

        return (random.random() < bid_prob, random.random() < bid_prob * alone_given_bid_prob, random.choice(elig_suits))
