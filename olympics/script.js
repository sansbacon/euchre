...
dots.forEach((dot, index) => {
  let time;
  if (index === 0) time = 4;       // 200F_1 lane
  else if (index === 1) time = 2;   // 200F_2 lane
  else time = getRandomTime();      // Random time between 3 and 4 for others

  animateDot(dot, time, poolWidth);
});
